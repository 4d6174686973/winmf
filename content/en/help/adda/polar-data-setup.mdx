...
``Copy to directivity`` editor directly transfers the measurement to the [Edit directivity](/en/help/directivity/edit-directivity) section.

``Plot directivity`` creates a graphic from the measuring results.

The inputs from the fields ``Manufacturer, Loudspeaker type`` and ``Configuration`` under ``Measurement info`` are transferred directly to the corresponding section in [Edit directivity](/en/help/directivity/edit-directivity).
